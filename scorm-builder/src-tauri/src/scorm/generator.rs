...
pub async fn generate_scorm_package(
    ...
    let output_path = temp_dir.path().join(format!("{}.zip", sanitize_filename(&request.course_metadata.title)));
    ...
    if request.generated_files.is_empty() {
        return Err("No generated files provided. SCORM generation must be done through JavaScript.".to_string());
    }
    ...
    for gen_file in &request.generated_files {
        ...
        if gen_file.is_binary {
            ...
            let content = general_purpose::STANDARD.decode(&gen_file.content)
    ...
    let mut media_extension_map: std::collections::HashMap<String, String> = std::collections::HashMap::new();
    ...
    let media_resources = collect_media_resources(&request.project_id, &request.course_content, &media_extension_map).await?;
    ...
    let media_ids: std::collections::HashSet<String> = extract_all_media_ids(&request.course_content);
    
    println!("[SCORM] Found {} media IDs in course content", media_ids.len());
    ...
    if base_path.exists() {
        ...
        println!("[SCORM] WARNING: Media directory does not exist: {}", base_path.display());
    }
    ...
    for media_id in &media_ids {
        ...
        if is_large {
            ...
            if file_path.exists() {
                ...
                println!("[SCORM] Added streamable resource: {} -> media/{}{}", file_path.display(), media_id, extension);
            } else {
                println!("[SCORM] WARNING: Media file not found: {}", file_path.display());
            }
        }
    }
            ...
            for entry in entries {
                if let Ok(entry) = entry {
                    ...
                    if file_name_str.ends_with(".bin") {
                        ...
                        let (extension, should_stream) = if let Some(ext) = media_extension_map.get(media_id) {
                            // Use the extension provided by JavaScript
                            let is_large_file = match ext.as_str() {
                                ".svg" => false, // SVG files are text, usually small
                                ".vtt" => false, // Caption files are text, always small
                        ...
                        if should_stream {
                            ...
                            println!("[SCORM] Added missing media from directory: {} -> media/{}{}", file_path.display(), media_id, extension);
                        }
                    }
                }
            }
...
fn extract_all_media_ids(course_content: &serde_json::Value) -> std::collections::HashSet<String> {
    ...
    fn extract_from_page(page: &serde_json::Value, media_ids: &mut std::collections::HashSet<String>) {
...
async fn collect_media_resources(
    ...
    // Helper function to extract media IDs from a page
    fn extract_media_from_page(page: &serde_json::Value, media_ids: &mut std::collections::HashSet<String>) {
        if let Some(audio_id) = page.get("audioId").and_then(|v| v.as_str()) {
            media_ids.insert(audio_id.to_string());
        }
        if let Some(caption_id) = page.get("captionId").and_then(|v| v.as_str()) {
...
pub fn stream_file_to_zip<W: Write + Seek>(
    ...
    let options = zip::write::FileOptions::default()
        .compression_method(zip::CompressionMethod::Stored); // No compression for media files
