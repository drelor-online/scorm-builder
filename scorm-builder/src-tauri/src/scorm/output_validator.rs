        ...
        self.validation_rules.insert(
            ...
            Box::new(|content| {
                ...
                if content.contains("min-height: 800px !important") {
                    return Err("Found problematic min-height that pushes footer off screen".to_string());
                }
            ...
            if name.starts_with("pages/topic-") && name.ends_with(".html") {
                ...
                if content.contains("knowledge-check-container") {
                    ...
                    if content.contains("fill-blank-") && !content.contains("kc-fill-blank") {
                        report.add_error(name.clone(), "Fill-in-blank input missing proper class".to_string());
                    }
                    
                    // Verify submit button
                    if content.contains("fill-blank-") && !content.contains("onclick=\"window.checkFillInBlank") {
                        report.add_error(name, "Fill-in-blank submit button missing proper onclick handler".to_string());
                    }
                }
            }
        }
...
impl ValidationReport {
    ...
    pub fn summary(&self) -> String {
        let mut summary = format!("Validation Report: {} success, {} errors\n", 
...
mod tests {
    ...
    fn test_navigation_validation() {
        ...
        let rule = validator.validation_rules.get("scripts/navigation.js").unwrap();
