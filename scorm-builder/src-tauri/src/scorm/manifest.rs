...
pub fn generate_manifest(options: &ManifestOptions) -> Result<String, String> {
    ...
    manifest_elem.push_attribute(("identifier", quick_xml::escape::escape(&options.course.identifier).as_ref()));
    manifest_elem.push_attribute(("version", quick_xml::escape::escape(&options.course.version).as_ref()));
    
    // Add namespace based on SCORM version
    match options.scorm_version.as_str() {
        "1.2" => {
            manifest_elem.push_attribute(("xmlns", "http://www.imsproject.org/xsd/imscp_rootv1p1p2"));
            manifest_elem.push_attribute(("xmlns:adlcp", "http://www.adlnet.org/xsd/adlcp_rootv1p2"));
        },
        _ => {
    ...
    writer.write_event(Event::Text(BytesText::new("ADL SCORM")))
    ...
    writer.write_event(Event::Start(BytesStart::new("schemaversion")))
    ...
    writer.write_event(Event::Text(BytesText::new(version_text)))
        .map_err(|e| format!("Failed to write schemaversion text: {}", e))?;
    writer.write_event(Event::End(BytesEnd::new("schemaversion")))
    ...
    writer.write_event(Event::Start(orgs_elem))
    ...
    writer.write_event(Event::Start(org_elem))
        .map_err(|e| format!("Failed to write organization start: {}", e))?;
    
    writer.write_event(Event::Start(BytesStart::new("title")))
        .map_err(|e| format!("Failed to write title start: {}", e))?;
    writer.write_event(Event::Text(BytesText::from_escaped(&options.course.title)))
    ...
    writer.write_event(Event::End(BytesEnd::new("organizations")))
