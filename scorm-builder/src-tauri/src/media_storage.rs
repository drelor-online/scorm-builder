...
pub fn store_media(
    ...
    println!("[media_storage] Storing media {} for project {} (extracted: {})", id, projectId, actual_project_id);
...
pub fn store_media_base64(
    ...
    // Extract actual project ID in case a path was passed
    let actual_project_id = extract_project_id(&projectId);
    println!("[media_storage] Storing media {} from base64 for project {} (extracted: {})", id, projectId, actual_project_id);
    
    // Decode base64 to Vec<u8>
    use base64::{Engine as _, engine::general_purpose};
    let data = general_purpose::STANDARD.decode(&dataBase64)
...
pub fn get_all_project_media(#[allow(non_snake_case)] projectId: String) -> Result<Vec<MediaData>, String> {
    // Extract actual project ID in case a path was passed
    let actual_project_id = extract_project_id(&projectId);
    println!("[media_storage] Loading all media for project {} (extracted: {})", projectId, actual_project_id);
...
pub fn delete_media(
    ...
    // Extract actual project ID in case a path was passed
    let actual_project_id = extract_project_id(&projectId);
    println!("[media_storage] Deleting media {} from project {} (extracted: {})", mediaId, projectId, actual_project_id);
...
pub fn get_media(
    ...
    // Extract actual project ID in case a path was passed
    let actual_project_id = extract_project_id(&projectId);
    println!("[media_storage] Getting media {} from project {} (extracted: {})", mediaId, projectId, actual_project_id);
...
mod tests {
    ...
    fn test_delete_media_with_scormproj_filename() {
        ...
        assert!(result.is_ok(), "Should be able to delete media with .scormproj filename");
    ...
    fn test_get_media_with_scormproj_filename() {
        ...
        // This should work after fix (currently will fail)
        assert!(result.is_ok(), "Should be able to get media with .scormproj filename");
    ...
    fn test_store_media_accepts_base64_data() {
        ...
        assert!(result.is_ok(), "store_media_base64 should succeed: {:?}", result);
    ...
    fn test_store_media_handles_large_base64_files() {
        ...
        let stored_data = fs::read(&data_path).unwrap();
        assert_eq!(stored_data.len(), large_size, "Stored data size should match");
    }
    ...
    fn test_backward_compatibility_with_vec_u8() {
        ...
        let _ = |id: String, project_id: String, data: Vec<u8>, metadata: MediaMetadata| -> Result<(), String> {
            store_media(id, project_id, data, metadata)
        };
