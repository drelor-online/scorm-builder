...
export const JSONImportValidator: React.FC<JSONImportValidatorProps> = ({ 
  ...
  const initialJson = initialData ? JSON.stringify(initialData, null, 2) : ''
  ...
  // Update jsonInput when initialData changes
  React.useEffect(() => {
    if (initialData) {
      const newJson = JSON.stringify(initialData, null, 2)
      setJsonInput(newJson)
      console.log('JSONImportValidator: Updated jsonInput from initialData', initialData)
    }
  }, [initialData])
  ...
  const handleClear = async () => {
    ...
    if (window.confirm('Warning: Clearing the JSON will delete all data on the following pages. Are you sure you want to continue?')) {
      setJsonInput('')
      setValidationResult(null)
      setIsLocked(false)
  ...
  const coursePreviewElement = validationResult?.isValid && validationResult.data ? (
    <CoursePreview 
      courseContent={validationResult.data} 
      courseSeedData={{
        courseTitle: validationResult.data.welcomePage?.title || 'Course Preview',
        difficulty: 3,
        customTopics: [],
        template: 'None',
        templateTopics: []
