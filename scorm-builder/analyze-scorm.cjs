...
async function loadProjectData() {
  ...
  console.log('Project loaded:', projectData.project?.name || 'Unknown');
  
  // Log the top-level structure
  console.log('\nTop-level keys:', Object.keys(projectData));
...
async function analyzeProjectContent(projectData) {
  ...
  console.log('\nContent structure keys:', Object.keys(content || {}));
  
  // Try different possible structures
  const metadata = content?.metadata || projectData.course_data || {};
  const topics = content?.topics || [];
  
  console.log('\nCourse Metadata:');
  console.log(`  Title: ${metadata.title || projectData.project?.name || 'Unknown'}`);
